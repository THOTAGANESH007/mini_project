...
const PaymentHistory = () => {
  ...
  return (
    <div style={{minHeight:"100vh"}} className="container mx-auto p-6 max-w-6xl">
                ...
                {paymentData.length === 0 ? (
                  ...
                  paymentData.map((item) => (
                    <tr key={item.sno} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.sno}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.billType}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">₹{item.amount}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(item.payment_status)}`}>
                          {item.payment_status}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.payment_method}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.dueDate}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.user_email}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.user_phone}</td>
                    </tr>
                  ))
                )}
              </tbody>
